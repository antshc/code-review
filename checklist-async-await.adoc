...
* Limit degree of parallelism if the downstream resource can’t handle full fan-out

== 17. Don’t Do Heavy CPU Work Inside Async Methods
...
=== ❌ Bad Example (captures entire object and fields)
[source,csharp]
----
...
=== ✔ Good Example (captures only required data)
...
public class FileProcessor
{
    ...
    {
        // Only the parameters are captured — not `this` and not `_largeBuffer`
...
=== ✔ Better Example (captures only required data)
[source,csharp]
----
...
= Resources
- https://learn.microsoft.com/en-us/archive/msdn-magazine/2013/march/async-await-best-practices-in-asynchronous-programming?WT.mc_id=DT-MVP-5000058#configure-context[Async/Await Best Practices – MSDN Magazine]
- https://learn.microsoft.com/en-us/archive/msdn-magazine/2015/july/async-programming-brownfield-async-development?WT.mc_id=DT-MVP-5000058[Async/Await Best Practices – MSDN Magazine 2015]
- https://blog.stephencleary.com/2022/03/cancellation-4-polling.html[Cancellation, Part 4: Polling 2022]
- https://github.com/jeremiahflaga/concurrency-in-csharp-cookbook-2e/tree/main/ConcurrencyInCSharpCookbook2e[github concurrency-in-csharp-cookbook-2e]
- https://www.albahari.com/threading/[albahari threading]
